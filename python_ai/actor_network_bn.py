...
class ActorNetwork:
	...
	def create_network(self,state_dim,action_dim, state_input):
		...
		action_output = tf.multiply(tf.sigmoid(tf.matmul(layer2_bn,W3) + b3), tf.constant([-35.0, 35.0, 2000.0]))
	...
	def create_target_network(self,state_dim,action_dim,net, target_state_input):
		...
		action_output = tf.multiply(tf.sigmoid(tf.matmul(layer2_bn,target_net[4]) + target_net[5]), tf.constant([-35.0, 35.0, 2000.0]))
