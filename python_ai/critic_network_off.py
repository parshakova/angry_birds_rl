...
class CriticNetwork:
  ...
  def create_training_method(self):
    ...
    self.summary = tf.summary.scalar("loss", self.cost)
  ...
  def train_off(self,y_batch, state_var,state_batch,action_batch):
    self.time_step += 1
    d1 = {self.y_input:y_batch, self.action_input:action_batch}
    d2 = {i: d for i, d in zip(state_var, state_batch)}
    d1.update(d2)
    _, cost, summary = self.sess.run([self.optimizer, self.cost, self.summary],feed_dict=d1); return cost, summary
