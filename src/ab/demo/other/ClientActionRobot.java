...
public class ClientActionRobot {
	...
	public BufferedImage shoot(byte[] fx, byte[] fy, byte[] dx, byte[] dy, byte[] t1,
			byte[] t2, boolean polar, BufferedImage initial) {
	...
	public BufferedImage shootFast(int fx, int fy, int dx, int dy, int t1,
			int t2, boolean polar) {
		byte[] inbuffer = new byte[16];
		try {
			if (polar)
				out.write(ClientMessageEncoder.pFastshoot(intToByteArray(fx), intToByteArray(fy), intToByteArray(dx), intToByteArray(dy), intToByteArray(t1), intToByteArray(t2)));
			else
				out.write(ClientMessageEncoder.cFastshoot(intToByteArray(fx), intToByteArray(fy), intToByteArray(dx), intToByteArray(dy), intToByteArray(t1), intToByteArray(t2)));
			out.flush();
			in.read(inbuffer);
            Thread.sleep(9000);
            finalState = doScreenShot();
